
public static class AudioString
{
    // Audio filter
    public const string AUDIO_FILTER = "t:AudioClip";


    // Music
    public static class MusicString
    {
        public const string MUSIC_BACKGROUND_PATH = "Assets/Audio/MusicBackground";
        public const string NAME_MUSIC_BACKGROUND = "ThemeSong";
    }


    // Sound effect
    public static class SoundString
    {
        public const string SOUND_EFFECT_PATH = "Assets/Audio/SoundEffect";

        // Name sound effect
        public const string COLLECTED = "Collected";
        public const string ENEMY_ATTACK = "EnemyAttack";
        public const string EXPLODED = "Exploded";
        public const string GAME_OVER = "GameOver";
        public const string GAME_WIN = "GameWin";
        public const string JUMP = "Jump";
        public const string NEW_MAP = "NewMap";

    }
}

